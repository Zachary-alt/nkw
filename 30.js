...
function strParse(str1,str2){
    ...
    //2
    let tmp = '',index=0
    while(index<evenArr.length||index<oddArr.length){
        tmp+=evenArr[index]
        if(oddArr[index]) tmp+=oddArr[index]
        index++
    }
    str = tmp
//     console.log(233,str)
    ...
    for(let i=0;i<str.length;i++){
        if(/[0-9a-fA-F]/.test(str[i])){
            let rv=[...parseInt(str[i],16).toString(2).padStart(4,'0')].reverse().join('')
//             console.log(i,rv)
            let up = parseInt(rv,2).toString(16).toUpperCase()
            res+=up
        }else{
            res+=str[i]
        }
